...
export type ParseSignature<TSignature extends string, TStructs extends StructLookup | unknown = unknown> = (IsErrorSignature<TSignature> extends true ? TSignature extends ErrorSignature<infer Name, infer Parameters> ? {
    ...
    readonly outputs: Tail extends `${string}returns (${infer Returns})` ? ParseAbiParameters<SplitParameters<Returns>, {
        Modifier: FunctionModifier;
        Structs: TStructs;
    }> : readonly [];
...
export type _ParseFunctionParametersAndStateMutability<TSignature extends string> = TSignature extends `${infer Head}returns (${string})` ? _ParseFunctionParametersAndStateMutability<Trim<Head>> : TSignature extends `function ${string}(${infer Parameters})` ? {
    Inputs: Parameters;
